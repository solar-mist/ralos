...
Process::Process(uint64_t entry, Paging::AddressSpace addrspace, char* interpPath)
    :addrspace(addrspace), pid(pidCount++), fdTable((ProcFile*)malloc(sizeof(ProcFile) * FD_MAX)), interpPath(interpPath)
{
    state.BaseFrame.rip = entry;
    state.BaseFrame.rsp = (uint64_t)VMM::GetPages(&addrspace, 64, 7) + (4 * PAGE_SIZE) - 1;
